...
void userinit(void)
{
  ...
  p = allocproc(); // 设置进程从forkret开始执行，forkret 会设置返回到用户空间的中断函数（uservec in trampoline.S），及
  initproc = p;

  // allocate one user page and copy init's instructions
  // and data into it.
  uvminit(p->pagetable, initcode, sizeof(initcode));
  k_vminit(p->kpagetable, initcode, sizeof(initcode));
...
void scheduler(void)
{
  ...
  for (;;)
  {
    ...
    {
      acquire(&p->lock);
      // 进程锁 防止同一个进程被不同的cpu调度，造成内核栈破坏，所有需要操作进程的位置都需要先获取锁，因为acquire 的逻辑包含了关中断，可以保证只存在cpu间的竞争行为
      /*

     因为存在调度的行为，进程锁的acquire 与release 并不是在同一位置进行的
     例如：
     scheduler -》acquire -》swtch -》（yield 后段 或forkret release） -》中断或睡眠（yield -》acquire -》sched-》swtch-》release  重复此循环
    
     简单一句： 进程切换后会先释放锁（进入scheduler，或进入本进程上次的内核代码（swtch后指令）
